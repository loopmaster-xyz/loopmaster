...
export const extra: [string, Definition][] = [
  ...
  ['range', {
    ...
    name: 'range',
    category: 'utilities',
    description: ['Returns array of integers from start to end (inclusive). Supports descending.'],
    return: 'array',
    parameters: [
      { name: 'start', description: ['Start value (floored).'] },
      { name: 'end', description: ['End value (floored).'] },
    ],
  }],
  ['print', {
    type: 'function',
  ...
  ['play', {
    ...
    category: 'utilities',
    description: [
      'Multi-voice playback. Events: [hz, trig, from, id, glide?]. Callback receives { hz, trig }. trig: >0 onset, 1 sustained, 0 releasing.',
    ],
    return: 'signal',
    parameters: [
      { name: 'events', description: ['[hz, trig, from, id, glide?] or array of such.'] },
      { name: 'cb', description: ['Callback ({ hz, trig }) -> signal.'], type: 'function' },
      { name: 'voices', description: ['Number of voices.'], default: 1 },
      { name: 'glide', description: ['Default glide time in seconds.'], default: 0 },
    ],
  }],
  ['midiToHz', {
    type: 'function',
  ...
  ['ntof', {
    ...
    name: 'ntof',
    category: 'utilities',
    description: ['Alias for midiToHz. Converts midi note to Hz.'],
    return: 'hz',
    parameters: [
      {
        name: 'note',
  ...
  ['db', {
    ...
    name: 'db',
    category: 'mixing',
    description: ['Convert decibels to linear gain multiplier (10^(dB/20)).'],
    return: 'number',
    parameters: [{ name: 'x', description: ['Decibels.'] }],
  }],
  ['uni', {
    type: 'function',
    name: 'uni',
    ...
    return: 'signal',
    parameters: [{ name: 'x', description: ['Bipolar signal -1..1.'] }],
  }],
  ['bi', {
    type: 'function',
    name: 'bi',
    ...
    return: 'signal',
    parameters: [{ name: 'x', description: ['Unipolar signal 0..1.'] }],
  }],
  ['crossfade', {
    type: 'function',
    name: 'crossfade',
    category: 'mixing',
    description: ['Crossfade between two signals. t 0=a, 1=b.'],
    return: 'signal',
    parameters: [
      { name: 'a', description: ['First signal.'] },
      { name: 'b', description: ['Second signal.'] },
      { name: 't', description: ['Crossfade position 0..1.'] },
    ],
  }],
  ['semis', {
    type: 'function',
    name: 'semis',
    category: 'mixing',
    description: ['Convert semitones to frequency multiplier (2^(semitones/12)).'],
    return: 'number',
    parameters: [{ name: 'x', description: ['Semitones.'] }],
  }],
  ['stereo', {
    type: 'function',
    name: 'stereo',
    category: 'mixing',
    description: ['Convert mono signal to stereo, optionally with delay-based widening.'],
    return: 'array',
    parameters: [
      { name: 'in', description: ['Mono input signal.'] },
      { name: 'width', description: ['Delay in seconds for right channel widening.'], default: 0 },
    ],
  }],
  ['mono', {
    type: 'function',
    name: 'mono',
    category: 'mixing',
    description: ['Convert stereo [L,R] to mono by averaging channels.'],
    return: 'signal',
    parameters: [{ name: 'LR', description: ['Stereo pair [L, R].'] }],
  }],
  ['stereowidth', {
    type: 'function',
    name: 'stereowidth',
    category: 'mixing',
    description: ['Adjust stereo width using mid-side processing. 1=normal, 0=mono, >1=wider.'],
    return: 'array',
    parameters: [
      { name: 'LR', description: ['Stereo pair [L, R].'] },
      { name: 'width', description: ['Width factor.'], default: 1 },
    ],
  }],
  ['widen', {
    type: 'function',
    name: 'widen',
    category: 'mixing',
    description: ['Widen stereo by delaying high frequencies in right channel.'],
    return: 'array',
    parameters: [
      { name: 'LR', description: ['Stereo pair [L, R].'] },
      { name: 'seconds', description: ['Delay for high freq right.'], default: 0.0001 },
    ],
  }],
  ['pan', {
    type: 'function',
    name: 'pan',
    category: 'mixing',
    description: ['Pan stereo signal. 0=left, 0.5=center, 1=right.'],
    return: 'array',
    parameters: [
      { name: 'LR', description: ['Stereo pair [L, R].'] },
      { name: 'balance', description: ['Pan position 0..1.'], default: 0.5 },
    ],
  }],
  ['outs', {
    type: 'function',
  ...
  ['vocoder', {
    ...
    parameters: [
      { name: 'carrier', description: ['Carrier signal.'] },
      { name: 'modulator', description: ['Modulator signal.'] },
      { name: 'bands', description: ['Number of bands.'], default: 16 },
      { name: 'attack', description: ['Envelope attack.'], default: 0.008 },
      { name: 'release', description: ['Envelope release.'], default: 0.04 },
      { name: 'freqMin', description: ['Lowest band freq.'], default: 80 },
      { name: 'freqMax', description: ['Highest band freq.'], default: 7700 },
    ],
  }],
  ['grain', {
    type: 'function',
    ...
    parameters: [
      { name: 'speed', description: ['Speed 0..1.'], default: 1 },
  ...
  ['eq3', {
    ...
    name: 'eq3',
    category: 'effects',
    description: ['3-band equalizer with low/mid/high controls.'],
    parameters: [
      { name: 'in', description: ['Input signal.'] },
      { name: 'low', description: ['Low band gain.'], default: 0 },
      { name: 'mid', description: ['Mid band gain.'], default: 0 },
      { name: 'high', description: ['High band gain.'], default: 0 },
      { name: 'lf', description: ['Low crossover freq.'], default: 500 },
      { name: 'mf', description: ['Mid center freq.'], default: 2000 },
  ...
  ['poly', {
    ...
    name: 'poly',
    category: 'synths',
    description: ['Polyphonic synth: count voices, trig selects, notescb(trig)->hz, voicecb(hz,trig)->signal.'],
    return: 'signal',
    parameters: [
      { name: 'count', description: ['Number of voices.'] },
      { name: 'trig', description: ['Trigger selecting voice.'] },
      { name: 'notescb', description: ['(trig) -> hz.'], type: 'function' },
      { name: 'voicecb', description: ['(hz, trig) -> signal.'], type: 'function' },
    ],
  }],
  ['karplus', {
    type: 'function',
  ...
  ['bdsynth', {
    ...
    name: 'bdsynth',
    category: 'synths',
    description: ['Kick/bass drum synth.'],
    parameters: [
      { name: 'base', description: ['Base frequency.'], default: '#1*o2' },
  ...
  ['bd', {
    ...
    name: 'bd',
    category: 'synths',
    description: ['Kick/bass drum. Recorded bdsynth.'],
    parameters: [
      { name: 'base', description: ['Base frequency.'], default: '#1*o2' },
      { name: 'punch', description: ['Punch frequency.'], default: '25000k' },
      { name: 'offset', description: ['Pitch offset.'], default: 0.0006 },
      { name: 'sampleOffset', description: ['Sample offset.'], default: 0 },
      { name: 'cutoff', description: ['Filter cutoff.'], default: '5k' },
      { name: 'q', description: ['Filter Q.'], default: 0.25 },
      { name: 'amp', description: ['Amp envelope (trig -> signal).'] },
      { name: 'fm', description: ['FM envelope (trig -> signal).'] },
  ...
  ['ch', {
    ...
    name: 'ch',
    category: 'synths',
    description: ['Closed hi-hat. Recorded hhsynth.'],
    parameters: [
  ...
  ['oh', {
    ...
    name: 'oh',
    category: 'synths',
    description: ['Open hi-hat. Recorded hhsynth.'],
    parameters: [
  ...
  ['sd', {
    ...
    name: 'sd',
    category: 'synths',
    description: ['Snare drum. Recorded sdsynth.'],
    parameters: [
      { name: 'seed', description: ['Random seed.'] },
  ...
  ['marimba', {
    ...
    name: 'marimba',
    category: 'synths',
    description: ['Marimba mallet instrument.'],
    parameters: [
      { name: 'hz', description: ['Frequency.'] },
      { name: 'trig', description: ['Trigger.'], default: 'every(1/8)' },
    ],
  }],
  ['piano', {
    type: 'function',
