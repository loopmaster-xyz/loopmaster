(function() {
	...
	function copyHistoryMetaToProgramShared(runtime, p, vm) {
		if (!p.historyMetaBuffers) return;
			...
			try {
				for (const p of programsById.values()) {
					...
					if (p.swapFadeRemaining > 0) {
						...
						runProgram(runtime, nyquist, piOverNyquist, bpmOverrideValueRef.value, bpmRef.value, p, p.slots[from], bufferLength, baseSampleCount, outputL, outputR, 1 - t$1, false);
